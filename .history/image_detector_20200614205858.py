...
ap.add_argument("-t", "--threshold", type=float, default=0.9,
                help="threshold")
ap.add_argument("-si", "--show-images", type=bool, default=True,
                help="threshold")
...
class Utility:
    ...
    def compare_images(self, im1, im2, imageA, imageB):
        ...
        self.totalCompare += 1
        if s >= tres:
            print("Image[{c1}] '{p1}' compared to Image[{c2}] '{p2}' Simility:{sim}".format(
                c1=im1['comp'], c2=im2['comp'], p1=im1['path'], p2=im2['path'], sim=str(s)))
            ...
            self.totalFound += 1
            companyTotal = self.companyResults.get(im1['comp'], 0)
            companyTotal += 1
...
print("{total} times compared. {found} similarity found by over {tr} treshold.".format(
    total=u.totalCompare, found=u.totalFound, tr=args['threshold']))

print('-------------------------------------------------------')
for key, value in u.companyResults.items():
    print(" ->  Company:{c} Total:{t}".format(c=key, t=value))
