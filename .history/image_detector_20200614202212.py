...
class Utility:
    ...
    def compare_images(self, path1, path2, imageA, imageB):
        ...
        if s >= tres:
            print("Image '{0}' comparet to '{1}' Simility:'{2}".format(
                [path1, path2, str(m), str(tres), str(s)]))
            twin = np.hstack([imageA, imageB])
            cv2.imshow('', twin)
            cv2.waitKey(0)
            self.searching = False
        elif self.searching is None or self.searching is not False:
            print('Searching...')
            self.searching = True
    ...
    try:
        ...
        for i in all_data:
            if i['path'] != image['path']:
                ...
                u = Utility()
                u.compare_images(image['path'], i['path'], p1, p2)
